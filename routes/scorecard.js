...
const { aiLimiter } = require("../utils/aiLimiter");

const router = express.Router();
const upload = multer({ storage: multer.memoryStorage() }); // Store in memory only

router.post("/", aiLimiter, upload.single("pdf"), uploadPDF);   // Upload + AI parse
router.get("/", getAllMatches);                      // Fetch all parsed matches
router.post('/validateStumpsReport', aiLimiter, upload.single('pdf'), validateStumpsReport);
router.get("/playerstat", playerstat);  
router.post("/playerstatadd", playerstatadd);  
router.post("/validate-player-names", aiLimiter, upload.single("pdf"), validatePlayerNamesFromPDF);
router.post("/extractPlayerNames", aiLimiter, upload.single("pdf"), extractPlayerNames);     
